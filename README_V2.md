...
`ProxyPoolV2` 是 `ProxyPool` 的重构版本，支持地区+协议过滤、权重优先、延迟排序、多代理源并发获取、混合检测，达到目标数量即停。
...
| `shuffle` | bool | False | 是否打乱代理顺序再检测，True=随机顺序，避免同一网站代理集中检测 |
...
```python
from utils.proxyPool.ProxyPoolV2 import ProxyPoolV2
...
```python
ProxyPoolV2.export()                    # proxies.txt
ProxyPoolV2.export(fmt='json')          # proxies.json（含延迟信息）
ProxyPoolV2.export(fmt='jsonl', filepath='D:\\proxies.jsonl')
```
...
| 名称 | 类名 | 权重 | API | 地区过滤 |
|------|------|------|-----|---------|
| 站大爷代理 | `ZdyProxyPool` | 3（最高） | http://www.zdopen.com/FreeProxy/Get/ | 支持（dalu参数） |
| 六六代理 | `SixSixProxyPool` | 2 | http://api.66daili.com/ | 不支持 |
| 云代理 | `YunProxyPool` | 1 | http://www.ip3366.net/free/ | 不支持 |
...
验证测速网站可用性...
================================================================================
✅测速网站可用：https://icanhazip.com/
✅测速网站可用：https://myip.ipip.net/
❌测速网站不可用：https://api.ip.sb/ip (ConnectTimeout)

================================================================================
地区：china | 协议：http | 代理源：全部 | 目标数量：10 | 权重模式：开启 | 随机检测：关闭
================================================================================
权重模式：站大爷代理(3) > 六六代理(2) > 云代理(1)
[站大爷代理] 获取到 200 条代理
[六六代理] 获取到 60 条代理
[云代理] 获取到 50 条代理
...
开始检测 290 条代理，目标 10 条，并发 8 线程
--------------------------------------------------------------------------------
❌ 无效 | 进度 1/290 | 可用 0/10
✅ http://5.6.7.8:3128 (230ms) | 进度 3/290 | 可用 1/10
✅ http://13.14.15.16:80 (150ms) | 进度 5/290 | 可用 2/10
...

✅已达目标数量 10

================================================================================
检测完成，共 10 条可用代理（按延迟排序）
  1. http://13.14.15.16:80 (150ms)
  2. http://5.6.7.8:3128 (230ms)
  ...
================================================================================
```
